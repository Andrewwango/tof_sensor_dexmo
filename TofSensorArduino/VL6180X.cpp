/*
 * This is the documentation for MHEtLive's VL6180X driver for the
 * Arduino platform.  It is designed specifically to work with the
 * MHEtLive VL6180X breakout.
 *
...
*/
/**************************************************************************/
boolean VL6180X::begin(uint8_t sensor_address, TwoWire *theWire) {
  //set up i2c
  ...
  //change address to desired address
  write8(0x212, sensor_address);
  _i2caddr = sensor_address;
  
  if (read8(VL6180X_REG_IDENTIFICATION_MODEL_ID) != 0xB4) {
    
    Serial.println(read8(VL6180X_REG_IDENTIFICATION_MODEL_ID));
    return false;
  }
...
uint8_t VL6180X::readAddress(void) {
  return read8(0x212);
}
/**************************************************************************/
/*! 
...
void VL6180X::loadSettings(void) {
    ...
    write8(0x010a, 0x20);       // Set the averaging sample period
    ...
    // Reduce max convergence time limit from 49 to 5
    write8(0x01c, 0x05);

    // Switch off ECE
    write8(0x02d, read8(0x02d) & 0xfe);
}
...
//These 4 functions break readRange up into bits
void VL6180X::singleShot_waitSensorReady(void) {
  while (! (read8(VL6180X_REG_RESULT_RANGE_STATUS) & 0x01)); 
}

void VL6180X::singleShot_startRange(void) {
  write8(VL6180X_REG_SYSRANGE_START, 0x01); 
}

boolean VL6180X::singleShot_checkResultReady(void) {
  return (read8(VL6180X_REG_RESULT_INTERRUPT_STATUS_GPIO) & 0x04);
}

uint8_t VL6180X::singleShot_readResult(void) {
  uint8_t range = read8(VL6180X_REG_RESULT_RANGE_VAL);
