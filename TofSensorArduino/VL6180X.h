...
class VL6180X {
 public:
  VL6180X();
  boolean begin(uint8_t sensor_address, TwoWire *theWire = NULL);
  uint8_t readRange(void);

  void singleShot_waitSensorReady(void);
  void singleShot_startRange(void);
  boolean singleShot_checkResultReady(void);
  uint8_t singleShot_readResult(void);
